...
var Book_v6 = {
    ...
    ready: function(){
	   ...
	   if( RTL == "false" ){		   
		   ...
		   $("#fb6-book > div:odd").addClass("even");
		   $("#fb6-book > div:even").addClass("odd");		   
	   }
       ...
       $(window).bind('orientationchange resize', function(event){
		...
		//reverse book
		$( $('#fb6-book>div').get().reverse()  ).each(function(index,element) {
			    var item=$(element);
				...
				if( RTL == "true" ){
					  ...
					  var desc=$('span.fb6-description',item);
					  if( desc.index() ==0 ){
						//desc.appendTo(meta);
					  }else{
						//desc.prependTo(meta);
					  ...
					  ///for double
					  if( item.hasClass('fb6-double') ){
						if(  item.hasClass('fb6-first') ){
						     item.removeClass('fb6-first').addClass('fb6-second');
						}else if( item.hasClass('fb6-second') ){
							 item.removeClass('fb6-second').addClass('fb6-first');
						}
					  }
					 //add data for meta
				 	 if( index%2!=0 ){
